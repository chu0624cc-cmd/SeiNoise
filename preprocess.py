...
from tool import bandpass_filter, cosine_taper, detrend, taper
...
def whiten_fft(fft_data: dict, freqmin: float, freqmax: float,
    ...
    fs   = fft_data['fs']
    N    = fft_data['n_pts']
    Nfft = fft_data['n_fft']             # ✅ 取 Nfft

    # ✅ 修复：freqvec 必须基于 Nfft，而不是 N
    # 原来是 rfftfreq(N, d=1.0/fs)，这是错误的！
    # F.shape[0] = Nfft//2+1，必须与 freqvec 长度一致
    freqvec = rfftfreq(Nfft, d=1.0 / fs)
    ...
    padarr = np.arange(pad, dtype=float)
...
def whiten_fft_step(raw: dict,
                    ...
                    pad:             int   = 50) -> dict:
    """
    标准/平滑谱白化，直接接收 step2_time_norm 返回的字典。

    流程：
      x(时域) → rfft → whiten_fft → F_white（频域）
                                   ↓ irfft
                                 x_white（时域）

    Parameters
    ----------
    raw     : step2_time_norm 返回的字典，含 'x'/'fs'/'freqmin'/'freqmax' 等
    freqmin : 白化频带下限，None 时从 raw['freqmin'] 取
    freqmax : 白化频带上限，None 时从 raw['freqmax'] 取
    method  : 'standard'（Julia默认）/ 'smoothed'

    Returns
    -------
    字典，含：
      'x_white'      : 白化后时域信号，shape (N, n_windows)
      'F_white'      : 白化后频域信号，shape (Nfft//2+1, n_windows)
      'freq'         : 频率轴
      'n_fft'        : FFT 点数
      'n_pts'        : 原始点数 N
      'whitened'     : True
      'whiten_method': 'standard' 或 'smoothed'
    """
    ...
    print(f"  [whiten_fft_step] method={method}")
    print(f"  [whiten_fft_step] F_white={F_white.shape}  x_white={x_white.shape}")
...
def whiten_gauss_step(raw: dict,
                      gwidth_sec: float = 6.0,
    ...
    gwidth_pts = gwidth_sec * fs
    win        = _gaussian(N, std=gwidth_pts)
    tap        = cosine_taper(N, p=0.1)        # ← 修正为 p=0.1
    F_white    = np.zeros((Nfft // 2 + 1, x.shape[1]), dtype=complex)
    ...
    for i in range(x.shape[1]):
        ...
        # divisor：加高斯窗 + 去均值
        divisor  = x_col * win
        divisor -= np.mean(divisor)
        fd       = rfft(divisor, Nfft)

        # waterlevel
        wl = eps * np.mean(fd * np.conj(fd))

        # signal：去均值 + cosine taper
        x_proc  = x_col.copy()
        x_proc -= np.mean(x_proc)
        x_proc *= tap
        sf      = rfft(x_proc, Nfft)

        F_white[:, i] = sf * fd / (fd ** 2 + wl)

    x_white = irfft(F_white, Nfft, axis=0)[:N, :]

    print(f"  [whiten_gauss_step] gwidth={gwidth_sec}s ({gwidth_pts:.0f}pts)  eps={eps}")
    print(f"  [whiten_gauss_step] F_white={F_white.shape}  x_white={x_white.shape}")
...
def step3_whiten(raw: dict,
                 ...
                 eps:             float = 0.001) -> dict:
    """
    Step3：谱白化统一入口，直接调用对应白化函数。

    whiten_method:
      'standard' / 'smoothed' → whiten_fft_step()
      'gauss'                 → whiten_gauss_step()
    """
    ...
    if whiten_method is None:
        ...
        print(f"  [Step3] whiten_method=None，跳过白化")
        return {**raw,
                'x_white': x,
                'F_white': F,
                'freq': freq,
    ...
    elif whiten_method == 'gauss':
        return whiten_gauss_step(raw, gwidth_sec = gwidth_sec, eps = eps)

    else:
        return whiten_fft_step(raw,
                               freqmin         = freqmin,
                               freqmax         = freqmax,
...
def preprocess(raw: dict,
               ...
               whiten_method:    str   = 'smoothed',  # None→跳过白化
               smooth_half_win:  int   = 5,
               pad:              int   = 50,
               gwidth_sec:       float = 6.0,
               eps:              float = 0.001,
               whiten_freqmin:   float = None,
               whiten_freqmax:   float = None
               ) -> dict:


    _wfmin = whiten_freqmin if whiten_freqmin is not None else freqmin
    _wfmax = whiten_freqmax if whiten_freqmax is not None else freqmax
